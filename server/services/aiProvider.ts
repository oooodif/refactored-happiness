...
const providerStatus = {
  ...
  together: {
    available: !!togetherApiKey,
    rateLimited: false,
    lastError: null,
  },
  huggingface: {
    available: !!huggingfaceApiKey,
    rateLimited: false,
    lastError: null,
  },
  fireworks: {
    available: !!fireworksApiKey,
    rateLimited: false,
    lastError: null,
  },
  openrouter: {
...
const providers = {
  openai: {
    ...
    async generateLatex(
      ...
      return extractLatexFromResponse(
  ...
  groq: {
    ...
    async generateLatex(
      ...
      const systemPromptTokens = Math.ceil(
        LATEX_SYSTEM_PROMPT.split(/\s+/).length * 1.3,
      );
      ...
      if (this.totalTokensUsed + estimatedRequestTokens > this.MAX_TOKENS) {
        throw new Error(
          "Groq spending limit exceeded. Using fallback providers.",
        );
      }
      ...
      try {
        ...
        const tokensUsed =
          response.data.usage?.total_tokens || estimatedRequestTokens;
        this.totalTokensUsed += tokensUsed;
        console.log(
          `Groq token usage: ${this.totalTokensUsed}/${this.MAX_TOKENS}`,
        );
  ...
  for (const provider of providerChain) {
    try {
      ...
      if (
        ...
        providerStatus[provider].rateLimited
      ) {
        continue;
      }
      ...
      // Call the provider
      const latex = await providers[provider].generateLatex(
        prompt,
        defaultModel,
      );
      console.log(
        `Successfully generated LaTeX using ${providers[provider].name}`,
      );
